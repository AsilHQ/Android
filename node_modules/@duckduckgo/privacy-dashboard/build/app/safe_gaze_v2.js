...
async function getImageElements() {
    ...
    const processImage = (image, skipCheck = false) => {
        ...
        if (skipCheck || typeof image.complete !== 'undefined') {
            if (image.complete) {
                if (!hasMinRenderedSize(image)) return;
            } else { 
                const prevImgLoad = image.onload // Save the previous onload function
                image.onload = () => {
                    if (hasMinRenderedSize(image)) {
            ...
            if (hasMinRenderedSize(image) === false) return; // If the element is rendered but not of minimum size
        }
        ...
        image.setAttribute('isSent', 'true');
    ...
    const observeElement = (el, srcChanged=false) => {
        try {
            ...
            let bgImage = window.getComputedStyle(node, null).getPropertyValue('background-image')
            let match = bgImage.match(srcChecker);
            let xlink = el.getAttribute('xlink:href');

            if (match) { // to handle background images
                node.src = match[1];
                processImage(node);
            }
            else if (/^img$/i.test(el.tagName)) { // to handle img tags
                if (el.src?.length > 0) {
                    processImage(el);
                } 
            } 
