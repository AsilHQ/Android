...
function handleLongPress(imgElement) {
    let timer;
...
function blurImage(image) {
    image.style.filter = `blur(${window.blurIntensity * 10}px) grayscale(${window.blurIntensity * 100}%)`;
...
function updateBluredImageOpacity() {
    ...
    blurredElements.forEach(element => {
        element.style.filter = `blur(${window.blurIntensity * 10}px) grayscale(${window.blurIntensity * 100}%)`;
    });
}
