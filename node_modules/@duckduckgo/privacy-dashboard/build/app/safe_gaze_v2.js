...
async function getImageElements() {
    ...
    const processImage = (htmlElement, src, srcChanged=false, skipCheck = false) => {
        if (htmlElement.getAttribute('isSent') && !srcChanged) return; // for ext
        ...
        htmlElement.setAttribute('isSent', 'true');
    ...
    const observeElement = (el, srcChanged=false) => {
        try {
            ...
            if (match) { // to handle background images
                src = match[1];
                processImage(el, src, srcChanged);
            }
