...
function blurImage(image) {
    ...
    const blurValue = Math.min(window.blurIntensity * 10, 5); // Max blur is 5px
    const grayscaleValue = Math.min(window.blurIntensity * 100, 100); // Max grayscale is 100%
    const contrastValue = Math.max(Math.min(window.blurIntensity * 2, 2), 1); // Max contrast is 2, Min is 1
    const brightnessValue = Math.max(Math.min(window.blurIntensity * 2, 2), 1); // Max brightness is 2, Min is 1

    // Apply the calculated filter values to the image
    image.style.filter = `blur(${blurValue}px) grayscale(${grayscaleValue}%) contrast(${contrastValue}) brightness(${brightnessValue})`;
