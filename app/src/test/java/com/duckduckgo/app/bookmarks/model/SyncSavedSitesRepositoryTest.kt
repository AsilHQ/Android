...
class SyncSavedSitesRepositoryTest {
    ...
    @Test
    fun whenDeduplicatingFavouritesThenRemoteFavouriteReplacesLocal() {
        // given a few local favourites favourite
        repository.insert(favourite1, favouritesRoot.entityId)

        // when replacing with a remote one
        val remoteFavourite1 = favourite1.copy(id = "remoteFavourite")
        repository.replaceFavourite(remoteFavourite1, favourite1.id, favouritesRoot.entityId)

        // entities have been replaced
        Assert.assertTrue(savedSitesEntitiesDao.entityById(favourite1.id) == null)
        Assert.assertTrue(savedSitesEntitiesDao.entityById(remoteFavourite1.id) != null)

        // and entity is still a bookmark
        val relations = savedSitesRelationsDao.relationsByEntityId(remoteFavourite1.id)
        Assert.assertTrue(relations.filter { it.folderId == SavedSitesNames.BOOKMARKS_ROOT }.isNotEmpty())
    }

    @Test
    fun whenDeduplicatingFavouritesThenRemoteFavoriteReplacesLocalAndKeepsOrder() {
        // given a few local favourites favourite
        repository.insert(favourite1, favouritesRoot.entityId)
        repository.insert(favourite2, favouritesRoot.entityId)

        Assert.assertTrue(repository.getFavoriteById(favourite1.id, favouritesRoot.entityId)?.position == 0)
        Assert.assertTrue(repository.getFavoriteById(favourite2.id, favouritesRoot.entityId)?.position == 1)
        ...
        // entities have been replaced
        Assert.assertTrue(repository.getFavoriteById(remoteFavourite1.id, favouritesRoot.entityId)?.position == 0)
        Assert.assertTrue(repository.getFavoriteById(favourite2.id, favouritesRoot.entityId)?.position == 1)
    ...
    fun whenReplacingBookmarkWithBookmarkFromAnotherFolderThenDataIsUpdated() {
        ...
        repository.replaceFolder(rootFolder, emptyList())
        repository.replaceFolder(subFolder, listOf(updatedBookmark.id))
        repository.replaceBookmark(updatedBookmark, bookmark.id)
    ...
    @Test
    fun whenBookmarksRootFolderModifiedAfterThresholdThenGetModifiedSinceReturnsIt() {
        val twoHoursAgo = OffsetDateTime.now(ZoneOffset.UTC).minusHours(2)
        ...
        val modifiedEarlierBookmarks = BookmarkTestUtils.givenSomeBookmarks(5, DatabaseDateFormatter.iso8601(oneHourAgo))
        savedSitesEntitiesDao.insertList(modifiedEarlierBookmarks)
        ...
        assertTrue(modifiedSinceFolders.filter { it.id == rootFolder.id }.isNotEmpty())
    }

    @Test
    fun whenBookmarksRootFolderModifiedBeforeThresholdThenGetModifiedSinceDoesNotContainIt() {
        val twoHoursAgo = OffsetDateTime.now(ZoneOffset.UTC).minusHours(2)
        val rootFolder = BookmarkFolder(
            id = SavedSitesNames.BOOKMARKS_ROOT,
            name = "root",
        ...
        assertTrue(modifiedSinceFolders.filter { it.id == rootFolder.id }.isEmpty())
    }
